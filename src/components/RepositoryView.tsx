...
interface RepositoryViewProps {
  ...
  onEditReport?: (report: OIMLTestReport) => void;
...
export const RepositoryView: React.FC<RepositoryViewProps> = ({
  ...
  const filtered = reports.filter((r) => {
    if (!r || !r.id || !r.instrument) return false;
    const matchesSearch =
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {onResetRepository && (
              <button
                ...
                title="Restore default benchmark samples (Retail Scale, Failing Corner Scale, Lab Balance)"
                className="ml-2 text-xs px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 rounded-lg flex items-center gap-1 transition-colors cursor-pointer"
              >
                <RotateCcw className="w-3.5 h-3.5 text-slate-500" />
                <span>Reset Benchmark Presets</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((r) => {
                const wPass = r.weighingTestPassed ?? (Array.isArray(r.weighingTest) && r.weighingTest.length > 0 && r.weighingTest.every((p) => Boolean(p.passOverall)));
                const ePass = r.eccentricityTestPassed ?? (Array.isArray(r.eccentricityTest) && r.eccentricityTest.length > 0 && r.eccentricityTest.every((p) => Boolean(p.pass)));
                const rPass = r.repeatabilityTestPassed ?? Boolean(r.repeatabilityTest && r.repeatabilityTest.pass);
                const allSubTestsPass = wPass && ePass && rPass;
                const isPass = r.overallVerdict === 'PASS' || allSubTestsPass;
                  ...
                  <tr key={r.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3 px-3 text-center">
                      ...
                      <div className="flex items-center justify-center gap-1 mt-1 text-[10px] font-mono">
                        <span className={wPass ? 'text-emerald-700 font-semibold' : 'text-rose-600 font-bold'} title="Weighing Performance">W:{wPass ? '✓' : '✗'}</span>
                        <span className="text-slate-300">|</span>
                        <span className={ePass ? 'text-emerald-700 font-semibold' : 'text-rose-600 font-bold'} title="Eccentricity Corner Test">E:{ePass ? '✓' : '✗'}</span>
                        <span className="text-slate-300">|</span>
                        <span className={rPass ? 'text-emerald-700 font-semibold' : 'text-rose-600 font-bold'} title="Repeatability Run">R:{rPass ? '✓' : '✗'}</span>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-1.5 flex-wrap">
                        ...
                        {onEditReport && (
                          <button
                            ...
                            title="Edit / Re-test measurements in Wizard"
                            className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md font-medium text-xs flex items-center gap-1 transition-colors cursor-pointer"
                          >
